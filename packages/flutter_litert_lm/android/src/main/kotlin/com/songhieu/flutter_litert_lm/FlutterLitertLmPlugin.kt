    ...
    private fun handleCreateEngine(call: MethodCall, result: Result) {
        scope.launch {
            try {
                ...
                val maxNumTokensArg = call.argument<Int>("maxNumTokens")

                val backend = parseBackend(backendName)
                // Local fork patch: upstream 0.3.0 reads visionBackend/audioBackend/
                // maxNumTokens from the method-channel args but drops them on the
                // floor here, so multimodal models like Gemma-3n null-deref inside
                // nativeSendMessage when an ImageBytes content is sent, and the
                // KV cache always allocates at the model's full context length
                // (multi-hundred-MB on 8k-ctx models). Wire all three through
                // to the upstream EngineConfig.
